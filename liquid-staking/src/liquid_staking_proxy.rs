    ...
    Gas: TxGas<Env>,
{
    ...
    /// Allows users to stake EGLD in exchange for sEGLD.
    ///
    /// # Notes
    ///
    /// - Whitelisted callers will be able to delegate to specific Delegation smart contracts.
    ...
    /// Allows the admin or trusted delegators to run the delegation algorithm and select a Delegation smart contract
    /// based on its current configuration. However, this endpoint does not automatically perform the delegation.
    /// Instead, anyone can perform the delegation at any given point in time using the `delegatePendingAmount` public
    /// endpoint.
    ///
    /// # Arguments
    ///
    /// - `opt_egld_amount`: The optional EGLD amount to delegate. If not specified, it will delegate the entire pre
    ///   delegated amount.
    ///
    /// # Notes
    ///
    /// - Can only be called by the admin or trusted delegators.
    ///
    pub fn delegate_pre_delegated_amount<
    ...
    >(
        ...
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("delegatePreDelegatedAmount")
            .argument(&opt_egld_amount)
            .original_result()
    }

    /// Allows the admin or trusted delegators to bypass the delegation algorithm and delegate to a specific Delegation
    /// smart contract. However, this endpoint does not automatically perform the delegation. Instead, anyone can perform
    /// the delegation at any given point in time using the `delegatePendingAmount` public endpoint.
    ///
    /// # Arguments
    ///
    ...
    ///
    pub fn delegate_pre_delegated_amount_to<
        Arg0: ProxyArg<ManagedAddress<Env::Api>>,
    ...
    >(
        ...
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("delegatePreDelegatedAmountTo")
    ...
    /// Adds a trusted delegator to the whitelist of trusted delegators.
    ///
    /// # Arguments:
    ///
    /// - `trusted_delegator` - the address of the trusted delegator to add
    ///
    /// # Notes:
    ///
    /// - can only be called by the admin
    /// - trusted delegators should not have been added before
    ///
    pub fn add_trusted_delegator<
        Arg0: ProxyArg<ManagedAddress<Env::Api>>,
    >(
        self,
        trusted_delegator: Arg0,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, ()> {
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("addTrustedDelegator")
            .argument(&trusted_delegator)
            .original_result()
    }

    /// Removes a trusted delegator from the whitelist of trusted delegators.
    ///
    /// # Arguments:
    ///
    /// - `trusted_delegator` - the address of the trusted delegator to remove
    ///
    /// # Notes:
    ///
    /// - can only be called by the admin
    /// - `trusted_delegator` must have already been trusted
    ///
    pub fn remove_trusted_delegator<
        Arg0: ProxyArg<ManagedAddress<Env::Api>>,
    >(
        self,
        trusted_delegator: Arg0,
        ...
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("removeTrustedDelegator")
            .argument(&trusted_delegator)
            .original_result()
    }
    ...
    /// Sets the instantaneous fee for instant undelegations. This fee is charged from the undelegated amount and it is
    /// distributed between the protocol and the rewards reserves. The fee is expressed in basis points.
    ///
    /// # Arguments
    ///
    /// - `fee` - the fee in basis points
    ///
    /// # Notes
    ///
    /// - can only be called by the admin
    ///
    pub fn set_instantaneous_fee<
        Arg0: ProxyArg<BigUint<Env::Api>>,
    >(
        self,
        fee: Arg0,
        ...
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("setInstantaneousFee")
            .argument(&fee)
            .original_result()
    }

    /// Sets the protocol share of the instantaneous fee. This share is expressed in basis points.
    ///
    /// # Arguments
    ///
    /// - `share` - the share in basis points
    ///
    /// # Notes
    ///
    /// - can only be called by the admin
    ///
    pub fn set_protocol_share<
        Arg0: ProxyArg<BigUint<Env::Api>>,
    >(
        self,
        share: Arg0,
    ...
    >(
        self,
        id: Arg0,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, Penalty<Env::Api>> {
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("getPenaltyById")
            .argument(&id)
            .original_result()
    }
    ...
    pub fn delegation_score_model(
        self,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, DelegationScoreModel<Env::Api>> {
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("getDelegationScoreModel")
            .original_result()
    }
    ...
    /// Stores the amount of EGLD that has been pre-delegated
    pub fn pre_delegated_amount(
        self,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, BigUint<Env::Api>> {
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("getPreDelegatedAmount")
            .original_result()
    }

    /// The instantaneous fee charged to users
    pub fn instantaneous_fee(
        self,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, BigUint<Env::Api>> {
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("getInstantaneousFee")
            .original_result()
    }

    /// The protocol share from the instantaneous fee
    pub fn protocol_share(
        self,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, BigUint<Env::Api>> {
        self.wrapped_tx
            .payment(NotPayable)
            .raw_call("getProtocolShare")
            .original_result()
    }

    /// Allows users to undelegate sEGLD in both instant and delayed modes. The delayed undelegation is performed by
    /// minting an undelegate NFT, which can be redeemed for EGLD after the unbond period has elapsed. The instant
    /// undelegation is performed by exchanging sEGLD for EGLD, which is sent to the caller. There is a fee for the
    /// instant undelegation, which is split between the protocol and the rewards reserve.
    ///
    /// # Notes
    ///
    /// - The instant undelegation has priority over the delayed undelegation.
    /// - If there is a delayed undelegation, the EGLD amount should be sufficient.
    ///
    pub fn optimized_undelegate<
        Arg0: ProxyArg<OptionalValue<BigUint<Env::Api>>>,
    >(
        self,
        opt_instant_egld_amount: Arg0,
    ) -> TxTypedCall<Env, From, To, (), Gas, MultiValue2<BigUint<Env::Api>, EsdtTokenPayment<Env::Api>>> {
        self.wrapped_tx
            .raw_call("optimizedUndelegate")
            .argument(&opt_instant_egld_amount)
            .original_result()
    }

    /// Allows users to instantly redeem sEGLD in exchange for EGLD. The EGLD is sent to the caller, and the sEGLD is
    /// burned. The EGLD amount is calculated based on the current exchange rate and the amount of sEGLD provided. A
    /// fee is charged for the instant undelegation, and it is split between the protocol and the rewards reserve. The
    /// fee is not charged in open mode.
    ///
    /// # Notes
    ///
    /// - There needs to be available EGLD at pre-delegation status for the instant undelegation.
    /// - The fee is charged in EGLD and is splitted between the protocol and the rewards reserve.
    /// - The fee is not charged in open mode.
    ///
    pub fn instantaneous_undelegate<
        Arg0: ProxyArg<OptionalValue<BigUint<Env::Api>>>,
    >(
        self,
        opt_egld_amount: Arg0,
        ...
        self.wrapped_tx
            .raw_call("instantaneousUndelegate")
            .argument(&opt_egld_amount)
            .original_result()
    }
    ...
    /// elapsed through the `unbond` endpoint. The paid sEGLD is burned.
    ///
    ...
    /// only if the undelegation mode is set to `Open`.
    ///
    ...
    /// Allows the admin or trusted delegators to delegate a Penalty to a new Delegation smart contract based on the
    /// current configuration of the delegation algorithm, avoiding the penalized Delegation smart contract. Similarly
    /// to `delegate`, it does not perform the delegation automatically, but it can be done by anyone at any given point
    /// in time using the `delegatePendingAmount` public endpoint.
    ///
    /// # Arguments
    ///
    /// - `penalty_id` - the penalty identifier
    ...
    /// Allows the admin to delegate a Penalty to a specific Delegation smart contract, avoiding the penalized
    /// Delegation smart contract. Similarly to `delegate`, it does not perform the delegation automatically, but it can
    /// be done by anyone at any given point in time using the `delegatePendingAmount` public endpoint.
    ///
    /// # Arguments
    ///
    /// - `penalty_id` - the penalty identifier
    ...
    /// A public endpoint that allows users to withdraw from penalties when the undelegation mode is set to `Open`.
...
#[type_abi]
#[derive(TopEncode, TopDecode)]
pub struct DelegationScoreModel<Api>
where
    Api: ManagedTypeApi,
{
    pub method: DelegationScoreMethod,
    pub min_tvl: BigUint<Api>,
    pub max_tvl: BigUint<Api>,
    pub min_apr: BigUint<Api>,
    pub max_apr: BigUint<Api>,
    pub omega: BigUint<Api>,
}
...
#[type_abi]
#[derive(TopEncode, TopDecode)]
pub struct Penalty<Api>
where
    Api: ManagedTypeApi,
{
    pub id: u64,
    pub withdrawn: bool,
    pub attributes: UndelegateAttributes<Api>,
}
